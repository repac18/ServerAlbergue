...
import {ACTUALIZAR_CLIENTE}         from '../../GraphQl/Mutations/Clientes';
...
class FormularioEdit extends Component {
  ...
  actualizarCliente = (e, actualizarCliente) => {
    e.preventDefault ();
    //insertamos en la base de datos
    this.base.swal2.create ('edit').then (result => {
      if (result.value){
        actualizarCliente ().then (data => {
          this.ClearState ();
  ...
  render () {
    ...
    return (
       <Fragment>
         <Mutation mutation={ ACTUALIZAR_CLIENTE } variables={ {input} }
                   key={ id }
           ...
           { (actualizarCliente, {loading, error, data}) => {
             return (
                <form autoComplete="off"
                   onSubmit={ e => this.actualizarCliente (e, actualizarCliente) }
                >
                  <div className="row">
