...
import {ACTUALIZAR_PACIENTE}         from '../../GraphQl/Mutations/Paciente';
...
class FormularioEdit extends Component {
  ...
  actualizarPacientes = (e, actualizarPacientes) => {
    e.preventDefault ();
    this.base.swal2.create('edit').then(result=>{
      if(result.value){
        actualizarPacientes ().then (data => {
  ...
  render () {
    ...
    return (
       <Fragment>
         <Mutation mutation={ ACTUALIZAR_PACIENTE } variables={ {input} }
                   key={ id }
                   onCompleted={ () => this.props.refetch ().then (() => {
                     this.props.history.push ('/Pacientes');
                   }) }>
           { (actualizarPacientes, {loading, error, data}) => {
             return (
                <form autoComplete="off"
                   onSubmit={ e => this.actualizarPacientes (e, actualizarPacientes) }
                >
                  <div className="row">
